...
//                    "store (" + store_name + ") found inactive in system");
    ...
    void open_and_close_store(){
        ...
        test_case = "store founder tries to close temporarily a store he already closed temporarily";
        close_store_temporarily_helper(suppose_to_throw, founder1, test_name, store_counter, test_case, f1_store1_id, f1_store1_name, 1, true);
     ...
     //   assertTrue(find_store(f1_f2_same_store_name, store_counter,true));
        res = add_prod_make_purchase_closed_store(f1_store2_id); // founder2 tries to make purchase from f2_store2
        assertTrue(check_was_exception(res), "User bought products from temporarily closed store successfully");
        ...
        assertTrue(check_was_exception(res), "User bought products from permanently closed store successfully");
    ...
    void view_store_management_information(){
        ...
        store_counter++;
        open_store_helper(!suppose_to_throw, founder, test_name, store_counter, test_case, "Store: " + test_name , 1);
        store_id = StoreController.get_instance().get_last_store_id();
