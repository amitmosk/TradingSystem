...
export default class HomeProductsTable extends Component{
    constructor(props) {
        ...
        this.columns = [
            ...
            { field: 'key_words', headerName:'key words',  width: 0, hide:true},
            {
              ...
              renderCell: (id) => (
                <>  
                <IconButton color="primary" aria-label="add to shopping cart" onClick={() => this.add_to_cart(id)}>
    ...
    async componentDidMount() {
        ...
        let products_list = []
        stores.value.map(st => st.inventory.map(p=>products_list.push(
        {id:p.product_id,name:p.name,category:p.category,price:p.price,quantity:0,store:p.store_id,key_words:p.key_words}
        )))
  ...
  add_to_cart = async (id) =>{
      ...
      let response = await this.productApi.add_product_to_cart(selected.store,selected.id,selected.quantity);
  ...
  return (
    ...
    <div style={{ height: 400, width: '100%' }}>
      <h1 style={{color: "white"}}>---------------------------------------------------------</h1>          <DataGrid rows={this.state.items} columns={this.columns}         
                            editMode='row'
                                            //   checkboxSelection
